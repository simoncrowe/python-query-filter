...
This package provides a functional API for filtering collection of
...
The remainder of the functions are used to construct predicates that
evaluate items or attributes within the filtered objects.
...
>>> results = (
        ...
        if data["LaunchTemplateData"]["NetworkInterfaces")[0].get("AssociatePublicIpAddress") is True
    )
```

One reason for this is that the item "AssociatePublicIpAddress" is always
present. The predicate returned by `q_item` will silently return `False` on
missing keys, whereas with the generator expression we need to call `get`
